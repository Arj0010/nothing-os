...
cp ~/.config/nothing-widgets/cleaner_core.py "$R/nothing-widgets/"
...
rm -f "$R/nothing-widgets/positions.json" "$R/nothing-widgets/notes.txt" \
      "$R/nothing-widgets/cleaner-history.jsonl" 2>/dev/null || true
rm -rf "$R/nothing-widgets/__pycache__" 2>/dev/null || true
